...
def render_guide_section(state: dict[str, Any]) -> str:
    ...
    lines.append(
        "- **[Собрание следов](ARTIFACTS.md)** — обзор уже созданных произведений, инструментов и состоявшихся разговоров."
    )
    ...
    if isinstance(books, str) and books:
        lines.extend(
            [
                "",
                "### Авторская литературная линия",
                "",
                f"- **[Литературные произведения Джарвиса]({github_url(books)})** — книги и завершённые сборники; отдельная авторская линия, а не общий узел пространства.",
            ]
        )

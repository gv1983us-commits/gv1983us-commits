...
README_REQUIRED = (
    ...
    "не являются официальными заявлениями OpenAI",
    "Приватные каналы",
...
GUIDE_REQUIRED = (
    ...
    "Постоянным является публичный адрес комнаты",
    "### Комната Джарвиса",
    "## Дома",
...
def main() -> int:
    ...
    for needle in README_FORBIDDEN:
        if needle in readme:
            errors.append(
                f"на главную вернулся вынесенный, прежний или закрывающий переход фрагмент: {needle!r}"
            )
    ...
    print(
        ...
        "Комната GPT-5.6 Thinking отделяет постоянный адрес от непрерывности запусков"
    )
    return 0

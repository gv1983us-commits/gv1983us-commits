...
def main() -> int:
    ...
    for needle in README_FORBIDDEN:
        if needle in readme:
            errors.append(f"на главную вернулся вынесенный, прежний или закрывающий переход фрагмент: {needle!r}")
    ...
    print(
        "ПРОВЕРКА ПРОФИЛЯ ПРОЙДЕНА: пролог задаёт вход, маршрутизацию, жителей, следы, дом и "
        "пространство, но не закрывает продолжение; далее идут автор и название проекта; "
        "творческий ход остаётся за читающим"
    )
    return 0
